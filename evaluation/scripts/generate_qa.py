...
5. 상황 부여의 유연성: 본인이 '컴퓨터공학부 소속이며 2026년도에 재학 중'이라는 페르소나를 가지되, 모든 질문에 "나 26년도 컴공인데~"라고 기계적으로 앵무새처럼 반복하지 마라.
6. ★정답(Ground Truth) 팩트 조작 절대 금지★: 조교(정답)는 반드시 제공된 원본 문서의 팩트(연도, 학과 제한, 일정 등)를 훼손하지 않고 있는 그대로 대답해야 한다.
7. 질문 유형 (반드시 아래 3가지 유형을 각각 1개씩 생성할 것):
   - 추론형 (Reasoning): 문서의 논리나 이유를 묻거나 결합해야 하는 질문.
   - 조건부 (Conditioning): 특정 조건 하에서 어떻게 적용되는지 묻는 질문.
   - 다중 문맥 (Multi-context): 최소 2개 이상의 청크에 있는 정보가 결합되어야만 대답할 수 있는 질문.
8. ★연도(Year) 필수 포함★: 무조건!! 생성되는 모든 질문에는 해당 문서의 기준 연도(예: "2023년", "24학년도", "올해 25년" 등)를 반드시 포함시켜라. 질문에 연도가 빠지면 절대 안 된다.
...
if __name__ == "__main__":
    ...
    # 300문제가 저장될 최종 파일명 (공지사항 전용임을 명시)
    save_path = os.path.join(save_dir, "notice_only_ragas_testset_300.json")
